...
Protected Module modFile
	...
	#tag Method, Flags = &h0
		Function GetISCCustomBlockFolder() As FolderItem
		  ...
		  if TargetMacOS = true then
		    ...
		    f = SpecialFolder.UserLibrary.Child("com.lunduke.armi")
		    ...
		    f = SpecialFolder.UserHome.Child("armi")
		...
		Sub InitTheDB(f as FolderItem)
		  ...
		  If theDB.CreateDatabaseFile then
		    if theDB.Connect() then
		      ...
		      theDB.SQLExecute("CREATE TABLE UIPlatforms (rowID integer NOT NULL PRIMARY KEY AUTOINCREMENT,iluiobjectsRowID integer,platformConstantNum integer,enabled boolean,height integer,text text,visible boolean,width integer,x integer,y integer,locktop integer,lockbottom integer,lockleft integer,lockright integer,color text,opacity double)")
	...
	#tag Method, Flags = &h0
		Sub LoadProject(f as FolderItem)
		  'frmProject.chartProject.EnableRendering = false
		            ...
		            If iluiWinRS <> Nil then
		              if iluiWinRS.RecordCount > 0 then // There is a window!
		                ...
		                do until uiPlatRS.EOF
		                  ...
		                  if uiPlatRS.Field("opacity") <> Nil then
		                    ilObjectBase(frmProject.chartProject.frames(frmProject.chartProject.frames.Ubound).tag).theWindowObject.Opacity(uiPlatRS.Field("platformConstantNum").IntegerValue) = uiPlatRS.Field("opacity").DoubleValue
		                  else
		                    ilObjectBase(frmProject.chartProject.frames(frmProject.chartProject.frames.Ubound).tag).theWindowObject.Opacity(uiPlatRS.Field("platformConstantNum").IntegerValue) = 1.0
		                  end if
		                ...
		                do until iluiObjsRS.EOF
		                  ...
		                  do until uiPlatRS.EOF
		                    ...
		                    if uiPlatRS.Field("opacity") <> nil then
		                      ilObjectBase(frmProject.chartProject.frames(frmProject.chartProject.frames.Ubound).tag).theUIObjects(uiBounder).Opacity(uiPlatRS.Field("platformConstantNum").IntegerValue) = uiPlatRS.Field("opacity").DoubleValue
		                    else
		                      ilObjectBase(frmProject.chartProject.frames(frmProject.chartProject.frames.Ubound).tag).theUIObjects(uiBounder).Opacity(uiPlatRS.Field("platformConstantNum").IntegerValue) = 1.0
		                    end if
		  ...
		  'frmProject.chartProject.EnableRendering = true
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Sub SafeProjectFileAs()
		  ...
		  dlg.SuggestedFileName="NewProject.armi"
		...
		Sub SaveContentFrame(theFrame as ContentFrame)
		  ...
		  if ilObjectBase(theFrame.tag).theWindowObject <> nil then
		    ...
		    for loopPlat = 0 to 9
		      ...
		      r.DoubleColumn("opacity") = ilObjectBase(theFrame.tag).theWindowObject.Opacity(loopPlat)
		  ...
		  if ilObjectBase(theFrame.tag).theUIObjects.Ubound > -1 then
		    for i = 0 to ilObjectBase(theFrame.tag).theUIObjects.Ubound
		      ...
		      for loopPlat = 0 to 9
		        ...
		        r.DoubleColumn("opacity") = ilObjectBase(theFrame.tag).theUIObjects(i).Opacity(loopPlat)
	...
	#tag Method, Flags = &h0
		Sub StartNewProject(ProjType as string)
		  ...
		  frmProject.chartProject.Width  = 1500
		  frmProject.chartProject.Height = 1500
		  ...
		  // These are needed in all project types
		  // Add App and AppQuit
		  frmProject.AddNewObject("Application", 20, 20)
		  frmProject.AddNewObject("AppLoop", 20, 200)
		  
		  if ProjType = "Hello World" then
