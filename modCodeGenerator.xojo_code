...
Protected Module modCodeGenerator
	#tag Method, Flags = &h0
		Sub BeginCodeGeneration(forTarget as integer)
		  ...
		  if TargetInProcess = app.kTargetWeb then
		    BeginCodeGenerationJavascript(forTarget)
		    BeginCodeGenerationJavascript(forTarget)
		  end if
		  
		  if TargetInProcess = app.kTargetDesktop then
		    BeginCodeGenerationJavascript(forTarget)
		    BeginCodeGenerationJavascript(forTarget)
		  end if
		  
		  if TargetInProcess = app.kTargetPhone then
		    BeginCodeGenerationJavascript(forTarget)
		    BeginCodeGenerationJavascript(forTarget)
		  end if
		  
		  if TargetInProcess = app.kTargetTablet then
		    BeginCodeGenerationJavascript(forTarget)
		    BeginCodeGenerationJavascript(forTarget)
		  end if
		  
		  
		  if TargetInProcess = app.kTargetAndroid then
		    BeginCodeGenerationAndroid(forTarget)
		    BeginCodeGenerationAndroid(forTarget)
		  end if
		  
		  if TargetInProcess = app.kTargetArduino then
		    BeginCodeGenerationArduino(forTarget)
		    BeginCodeGenerationArduino(forTarget)
		  end if
	...
	#tag Method, Flags = &h0
		Sub BeginCodeGenerationAndroid(forTarget as integer)
		  ...
		  AndroidMainJavaCode = ""
		  TargetInProcess = app.kTargetPhone
	...
	#tag Method, Flags = &h0
		Sub BeginCodeGenerationArduino(forTarget as integer)
		  dim i as integer
		  dim j as integer
		  ...
		  for each frame as contentFrame in frmProject.chartProject.frames
		    ...
		    'MsgBox ilObjectBase(frame.tag).DisplayName
		    // Generate the code
		    ilObjectBase(frame.tag).CallGenerateArduinoCode(1)
		    ...
		    for each content as contentObject in frame.contents
		      ...
		      if content.outputs.Ubound >= 0 then
		        ...
		        for each link as contentObject in content.outputs
		          ...
		          ilObjectBase(frmProject.chartProject.frames(0).tag).HelperGenerateArduinoCodePhase2(content, link)
	...
	#tag Method, Flags = &h0
		Sub BeginCodeGenerationJavascript(forTarget as integer)
		  ...
		  for each frame as contentFrame in frmProject.chartProject.frames
		    ...
		    'MsgBox ilObjectBase(frame.tag).DisplayName
		    // Generate the code
		    ilObjectBase(frame.tag).CallGenerateJavascriptWebCode(forTarget)
