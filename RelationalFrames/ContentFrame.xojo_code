...
Inherits CalculatedHeightRect
	...
	#tag Method, Flags = &h0
		Sub DrawAllLinks(g as Graphics, selectedFrame as contentFrame, XOffset as integer, YOffset as integer)
		  for each content as contentObject in contents
		    ...
		    if content.outputs.Ubound >= 0 then
		      for each link as contentObject in content.outputs
		        ...
		        drawLink(g, content.outputArea.x + 3 + self.x - XOffset, content.outputArea.y + 3  + self.y - YOffset, link.inputArea.x + 3 + link.parent.x - XOffset, link.inputArea.y + 3 + link.parent. y - YOffset, linksColor)
		      next
		    end if
		  next
		End Sub
