...
Inherits canvas
	...
	#tag Event
		Function MouseDown(X As Integer, Y As Integer) As Boolean
		  ...
		  for objectIndex = frames.Ubound DownTo 0
		    ...
		    objectHit = frame.MouseDown(x + areaX, y + areaY)
	...
	#tag Event
		Sub MouseDrag(X As Integer, Y As Integer)
		  ...
		  if selectedFrame <> nil then
		    // drag frame
		    'if x > 0 and y > 0 then
		    selectedFrame.Offset(x - lastMouseX - areaX, y - lastMouseY - areaY)
		  ...
		  elseif sourceObject <> nil then
		    ...
		    for objectIndex = frames.Ubound DownTo 0
		      objectHit = frames(objectIndex).MouseDrag(x + areaX, y + areaY)
		    ...
		    'xScroll = x - lastMouseX - areaX
		    
		    frmProject.scrlHor.Value = offsetX - x
		    frmProject.scrlVer.Value = offsetY - y
	...
	#tag Event
		Sub Paint(g As Graphics, areas() As REALbasic.Rect)
		  ...
		  areaX = frmProject.scrlHor.Value
		  areaY = frmProject.scrlVer.Value
		  areaWidth = frmProject.canScroller.Width + areaX
		  areaHeight = frmProject.canScroller.Height + areaY
		  
		  if EnableRendering = true then
		    
		    if TargetLinux then
		      
		      'g.DrawPicture GetPictureOfChart,0,0
		      
		      g.DrawPicture GetPictureOfChartWithSize(areaX, areaY, areaWidth, areaHeight),0,0
	...
	#tag Method, Flags = &h0
		Function GetPictureOfChart() As picture
		  ...
		  areaX = frmProject.scrlHor.Value - 200
		  areaY = frmProject.scrlVer.Value - 200
		  areaWidth = frmProject.canScroller.Width + areaX + 400
		  areaHeight = frmProject.canScroller.Height + areaY + 400
		  ...
		  for each frame as ContentFrame in frames
		    ...
		    frame.DrawAllLinks(p.Graphics, selectedFrame,0,0)
		    
		  next
		  
		  // now, draw the frames, and highlight the currently selected frame...
		  for each frame as ContentFrame in frames
		    'if frame.X > areaX and Frame.Y > areaY and frame.X < areaWidth and frame.Y < areaHeight then
	...
	#tag Method, Flags = &h0
		Function GetPictureOfChartWithSize(chartX as integer, chartY as integer, chartWidth as integer, chartHeight as integer) As picture
		  dim p as Picture
		  p = NewPicture(chartWidth, chartHeight, 32)
		  
		  // background
		  p.Graphics.ForeColor = rgb(159,170,181)
		  p.Graphics.FillRect 0, 0, p.Graphics.Width, p.Graphics.Height
		  ...
		  for each frame as ContentFrame in frames
		    ...
		    frame.DrawAllLinks(p.Graphics, selectedFrame, chartX, chartY)
		  ...
		  for each frame as ContentFrame in frames
		    ...
		    if selectedFrame = frame then
		      ...
		      if selectedFrame.rounded then
		        ...
		        p.Graphics.DrawRect frame.x - selectionWidth - chartX, frame.y - selectionWidth - chartY, frame.width + 2 * selectionWidth, frame.height + 2 * selectionWidth
		      end if
		      p.Graphics.PenWidth = 1
		      p.Graphics.PenHeight = 1
		      p.Graphics.ForeColor = &c0
		    end if
		    frame.Paint(p.Graphics, chartX, chartY)
		  ...
		  if sourceObject <> nil then
		    dim linkColor as color = &cffffff
		    if targetObject <> nil then linkColor = RelationalFrames.LinkColor
		    'drawLink(p.Graphics, sourceObject.outputArea.x + 3 + sourceObject.parent.x + chartX, sourceObject.outputArea.y + 3  + sourceObject.parent.y + chartY, lastMousex, lastMouseY, linkColor)
		    drawLink(p.Graphics, sourceObject.outputArea.x + 3 + sourceObject.parent.x - chartX, sourceObject.outputArea.y + 3  + sourceObject.parent.y - chartY, lastMousex + chartX, lastMouseY + chartY, linkColor)
		  end if
	...
	#tag Property, Flags = &h0
		EnableRendering As Boolean = true
	#tag EndProperty
