#tag Class
Protected Class ilObjectDictionaryLoad
Inherits ilObjectBase
	...
	#tag Event
		Sub GenerateJavaScriptWebCode(theOrigin as string, optional theTarget as integer)
		  ...
		  s = s + "var fromJSON = localStorage." + modVariables.VariableObjectFromName(OriginVariable).ISCClassLocalFlexName + ";" + EndOfLine
		  
		  s = s + modVariables.VariableObjectFromName(OriginVariable).ISCClassLocalFlexName  + " = JSON.parse(fromJSON);" + EndOfLine
		  
		  s = s + "//" + GetCodeName + "Done" + EndOfLine
		  
		  s = s + "}" + EndOfLine + EndOfLine
		  
		  setCodeToCallThisObjectForName("Load", GetCodeName + "();")
	...
	#tag Method, Flags = &h1000
		Sub Constructor()
		  ...
		  me.CodeNameBase = "iscDictLoad"
		  me.BGColor =  frmProject.kcolDictionary
		  me.DisplayName = "Load Dictionary"
		  me.DisplayCategory = "Dictionaries"
		  me.Title = "Load Dictionary"
		  me.Width = 90
		  
		  me.ObjectItems.Append "Load"
