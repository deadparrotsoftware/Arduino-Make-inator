...
Inherits ilObjectBase
	...
	#tag Event
		Sub GenerateArduinoCode(theOrigin as string, optional theTarget as integer)
		  ...
		  if Var1Enabled = true then
		    s = s.ReplaceAll("ISCVariable1",  modVariables.VariableObjectFromName(MathVariable1).ISCArduinoName)
		  end if
		  
		  if Var2Enabled = true then
		    s = s.ReplaceAll("ISCVariable2", modVariables.VariableObjectFromName(MathVariable2).ISCArduinoName)
		  end if
		  
		  if Var3Enabled = true then
		    s = s.ReplaceAll("ISCVariable3", modVariables.VariableObjectFromName(MathVariable3).ISCArduinoName)
		  end if
		  ...
		  ArduinoAddImports("//EndImports", TemplateImportsArduino)
		End Sub
	#tag EndEvent
	...
	#tag Event
		Sub GenerateJavaScriptWebCode(theOrigin as string, optional theTarget as integer)
		  ...
		  s = s + TemplateCodeHTML5Web + EndOfLine
	...
	#tag Method, Flags = &h0
		Sub ArduinoAddImports(ImportEnder as string, theImports as string)
		  dim s as string
		  ...
		  if importCount = 0 then
		    ...
		    if len(trim(theImports)) = 0 then
		      ...
		      ArduinoInsertCodeAtMarker(trim(theImports),ImportEnder)
		    end if
		    ...
		    for i = 1 to importCount
		      s = NthField(theImports, EndOfLine, i)
		      ArduinoInsertCodeAtMarker(trim(s), ImportEnder)
		    next
	...
	#tag Method, Flags = &h0
		Sub LoadBlockFromFile(f as FolderItem)
		  ...
		  rsBlock = myDB.SQLSelect("select * from TheCode WHERE target='Arduino'")
		  if rsBlock <> nil then
		    if rsBlock.RecordCount > 0 then
		      TemplateCodeArduino = rsBlock.Field("code").StringValue
		      if rsBlock.Field("importstatements") <> nil then
		        TemplateImportsArduino = rsBlock.Field("importstatements").StringValue
		      end if
		    end if
		  end if
	...
	#tag Method, Flags = &h0
		Sub SaveBlockToFile(f as FolderItem)
		  ...
		  r = new DatabaseRecord
		  r.Column("code") = TemplateCodeArduino
		  r.Column("importstatements") = TemplateImportsArduino
		  r.Column("target") = "Arduino"
		  myDB.InsertRecord("TheCode", r)
	...
	#tag Property, Flags = &h0
		TemplateImportsArduino As string
	#tag EndProperty
