#tag Class
Protected Class ilObjectAnimationOpacity
Inherits ilObjectBase
	...
	#tag Event
		Sub GenerateJavaScriptWebCode(theOrigin as string, optional theTarget as integer)
		  ...
		  dim tempILUI as ilUIObject
		  dim tempILWindow as ilObjectWindow
		  
		  if varControlName = "The Window" then
		    tempILWindow = modCodeGenerator.getilObjectWindowFromName(varWindowName)
		  else
		    tempILUI = modCodeGenerator.GetilUIObjectFromNameOnWindow(varControlName, varWindowName)
		  end if
		  ...
		  if varControlName = "The Window" then
		    s = s + "new Effect.Opacity('" + tempILWindow.theWindowObject.ISCClassLocalFlexName + "', { from: " + _
		    modVariables.VariableObjectFromName(MathVariable1).ISCClassLocalFlexName + _
		    ", to: "+ modVariables.VariableObjectFromName(MathVariable2).ISCClassLocalFlexName + _
		    ", duration: " + modVariables.VariableObjectFromName(MathVariable3).ISCClassLocalFlexName + " });" + EndOfLine
		    
		  else
		    s = s + "new Effect.Opacity('" + tempILUI.ISCClassLocalFlexName + "div', { from: " + _
		    modVariables.VariableObjectFromName(MathVariable1).ISCClassLocalFlexName + _
		    ", to: "+ modVariables.VariableObjectFromName(MathVariable2).ISCClassLocalFlexName + _
		    ", duration: " + modVariables.VariableObjectFromName(MathVariable3).ISCClassLocalFlexName + " });" + EndOfLine
		  end if
		  ...
		  setCodeToCallThisObjectForName("Change", GetCodeName + "();")
	...
	#tag Method, Flags = &h1000
		Sub Constructor()
		  ...
		  me.CodeNameBase = "iscOpacity"
		  me.BGColor =  frmProject.kcolAnimation
		  me.DisplayName = "Opacity"
		  me.DisplayCategory = "Animation"
		  me.Title = "Opacity"
		  me.Width = 90
		  
		  me.ObjectItems.Append "Change"
