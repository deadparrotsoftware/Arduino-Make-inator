#tag Class
Protected Class ilObjectNetGetURLParm
Inherits ilObjectBase
	...
	#tag Event
		Sub GenerateJavaScriptWebCode(theOrigin as string, optional theTarget as integer)
		  ...
		  s = s + "name = " + modVariables.VariableObjectFromName(MathVariable1).ISCClassLocalFlexName + ".replace(/[\[]/,""\\\["").replace(/[\]]/,""\\\]"");" + EndOfLine
		  s = s + "var regexS = ""[\\?&]""+name+""=([^&#]*)"";" + EndOfLine
		  s = s + "var regex = new RegExp( regexS );" + EndOfLine
		  s = s + "var results = regex.exec( window.location.href );" + EndOfLine
		  s = s + "if( results == null )" + EndOfLine
		  s = s + modVariables.VariableObjectFromName(MathVariable2).ISCClassLocalFlexName + " = '';" + EndOfLine + EndOfLine
		  s = s + "else" + EndOfLine
		  s = s + modVariables.VariableObjectFromName(MathVariable2).ISCClassLocalFlexName + "= results[1].replace(/%20/g,' ');" + EndOfLine
	...
	#tag Method, Flags = &h1000
		Sub Constructor()
		  ...
		  me.CodeNameBase = "iscURLParm"
		  me.BGColor =  frmProject.kcolDictionary
		  me.DisplayName = "Get URL Parameter"
		  me.DisplayCategory = "Network"
		  me.Title = "Get URL Parameter"
		  me.Width = 90
